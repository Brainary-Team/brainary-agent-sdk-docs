...
| **内置工具目录**（`read_file` / `write_file` / `edit_file` / `find_file` / `grep` 等） | 🟠（含 schema 精化缺口） | 随会话默认可用的一组内建工具；`bash` / `web_fetch` / `web_search` / `ask_user_question` / `agent`(🟠) 亦在此登记 | [内置工具目录](/sdk/builtin-tools) |
