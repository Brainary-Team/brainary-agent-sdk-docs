...
::: warning 规划状态
Python 版 brainary-agent-sdk **本身尚未实现**。本章记录的是**规划中的 Python 接口面**——与 Rust 版镜像、命名两侧一致，写法遵循 Python 惯用法。下表的「决策」列是**接口层面**的裁剪（与 Rust 版一致），「Python 实现」整体归路线图。
:::
