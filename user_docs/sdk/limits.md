...
brainary-agent-sdk v1 **规划交付（尚未实现）**：一次性 + 多轮入口、统一 `Options`、轻量自定义工具、进程内 resume、步级流、步边界中断、只读 transcript 导出。
...
| 跨进程 resume（从 transcript 续跑） | 🟠 | llmy 上游 | llmy 无「从历史重建 agent」的构造器；进程内 resume 规划由 `BrainaryClient` 支持 |
...
- **自动压缩策略**：上下文治理的自动压缩由 **core 拥有**并实现；`Options.auto_compact` 旋钮规划就位，落地后翻开 `Auto` / `Threshold(_)` 即可用。
...
| `billing_cap` / `ResultMessage.total_cost_usd` | 🟠 | 成本面：`billing_cap` 规划就位、`total_cost_usd` 恒 `None` 待上游，[成本](/sdk/messages#cost) |
...
| 接口 | 决策 | 一句理由 / 去处 |
| --- | --- | --- |
| **内置工具目录**（Read/Write/Edit/Bash/Glob/Grep/WebFetch/WebSearch/Agent 等，Brainary 原生名） | 🟠 | 编码智能体 SDK 的定义性面：文件类规划由 `FolderPrimitive` 装配（schema 待精化），shell/web/交互/任务/子 agent 均规划中，[内置工具目录](/sdk/builtin-tools) |
...
| 接口 | 决策 | 一句理由 / 去处 |
| --- | --- | --- |
| `connect` / `query` / `interrupt` / `disconnect` | 🟠 | 规划中，[两个入口](/sdk/query-and-client#full-method-face) |
...
> **Brainary 独有**：`step_once` / `revert`（手动单步回退）、`export_transcript`（只读快照）、`approx_context_tokens` / `context_window`（上下文检视）——均为 🟠【规划中，未完成】，见 [两个入口](/sdk/query-and-client#brainaryclient-透传方法)。
