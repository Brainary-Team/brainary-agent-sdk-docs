...
- 状态：**三项确定性优化已落地；元凶定位待在 MacBook 上验证**
...
- [ ] 用 7.2 证实／推翻第 4 节的元凶推断
- [ ] 若 `grid-field` 确为元凶且 C 已解决 → 收工
- [ ] 若 `noise-field` 的 `mix-blend-mode` 仍贵 → 再议（需权衡观感）
- [ ] 若 canvas 才是元凶 → 需要在**真 GPU 上**重做 canvas 侧的测量与优化，
      本文 3.3 的光栅化数据在那种情况下**不可用**
- [ ] 7.4 视觉验收
