...
下表状态标记表示**接口清单里的实现状态**（多为 Rust 版已实现 / 类型就位）；**Python 版本身尚未实现**,签名为设计形态。详见 [总览](/sdk-py/overview) 规划横幅。
:::
...
| **内置工具目录**（`read_file` / `write_file` / `edit_file` / `find_file` / `grep` 等） | 🟢（schema 精化 🟠) | 随会话默认可用的一组内建工具；`bash` / `web_fetch` / `web_search` / `ask_user_question` / `agent`(🟠) 亦在此登记 | [内置工具目录](/sdk-py/builtin-tools) |
